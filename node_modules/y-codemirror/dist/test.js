(function () {
  ...
  */
  ...
  /* istanbul ignore next */
  const uint32$1 = () => new Uint32Array(cryptoRandomBuffer$1(4))[0];
  ...
  const BOLD$1 = create$1();
  const UNBOLD$1 = create$1();
  const BLUE$1 = create$1();
  const GREY$1 = create$1();
  const GREEN$1 = create$1();
  const RED$1 = create$1();
  const PURPLE$1 = create$1();
  const ORANGE$1 = create$1();
  const UNCOLOR$1 = create$1();

  /**
   * @type {Object<Symbol,pair.Pair<string,string>>}
   */
  const _browserStyleMap$1 = {
    [BOLD$1]: create$2('font-weight', 'bold'),
    [UNBOLD$1]: create$2('font-weight', 'normal'),
    [BLUE$1]: create$2('color', 'blue'),
    [GREEN$1]: create$2('color', 'green'),
    [GREY$1]: create$2('color', 'grey'),
    [RED$1]: create$2('color', 'red'),
    [PURPLE$1]: create$2('color', 'purple'),
    [ORANGE$1]: create$2('color', 'orange'), // not well supported in chrome when debugging node with inspector - TODO: deprecate
    [UNCOLOR$1]: create$2('color', 'black')
  };
  ...
  /* istanbul ignore next */
  const computeLoggingArgs$1 = isNode ? computeNodeLoggingArgs$1 : computeBrowserLoggingArgs$1;
  ...
  const typeListDelete = (transaction, parent, index, length) => {
    ...
    if (length > 0) {
      throw create$6('array length exceeded')
    }
    if (parent._searchMarker) {
  ...
  class Item extends AbstractStruct {
    ...
    delete (transaction) {
      if (!this.deleted) {
        ...
        setIfUndefined$1(transaction.changed, parent, create$7).add(this.parentSub);
  ...
  const compare$1 = users => {
    ...
    ymapkeys.forEach(key => assert(hasProperty$1(userMapValues[0], key)));
    /**
     * @type {Object<string,any>}
     */
    const mapRes = {};
      ...
      const performChange = () => {
        ...
        for (let i = 0; i < event.delta.length; i++) {
          ...
          if (d.retain) {
            ...
            if (index < anchor) {
              anchor = max$1(anchor - d.delete, index);
            }
            if (index < head) {
              head = max$1(head - d.delete, index);
            }
    ...
    binding._mux(() => {
      binding.doc.transact(() => {
        if (changes.length > 1) {
          ...
          const delLen = change.removed.map(s => s.length).reduce(add$1) + change.removed.length - 1;
          if (delLen > 0) {
  ...
  const cmChanges = [
    ...
    (y, gen, cm) => { // replace text
      const insertPos = int32(gen, 0, cm.getValue().length);
      const overwrite = min$1(int32(gen, 0, cm.getValue().length - insertPos), 3);
  ...
  /* istanbul ignore next */
  const isBrowser$1 = typeof window !== 'undefined' && !isNode$1;
  /* istanbul ignore next */
  const isMac$1 = typeof navigator !== 'undefined' ? /Mac/.test(navigator.platform) : false;
   ...
   */
  /* istanbul ignore next */
  const getVariable$1 = name => isNode$1 ? undefinedToNull(process.env[name.toUpperCase()]) : undefinedToNull(varStorage.getItem(name));
