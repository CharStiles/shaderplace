(function () {
  ...
  const addToDeleteSet = (ds, client, clock, length) => {
    setIfUndefined$1(ds.clients, client, () => []).push(new DeleteItem(clock, length));
  };
  ...
  /* istanbul ignore next */
  const toBase64$1 = isBrowser ? toBase64Browser$1 : toBase64Node$1;

  /* istanbul ignore next */
  const fromBase64$1 = isBrowser ? fromBase64Browser$1 : fromBase64Node$1;
  ...
  const splitSnapshotAffectedStructs = (transaction, snapshot) => {
    const meta = setIfUndefined$1(transaction.meta, splitSnapshotAffectedStructs, create$4);
   ...
   * @return {Pair<L,R>}
   */
  const create$6 = (left, right) => new Pair(left, right);
  ...
  const _browserStyleMap = {
    [BOLD]: create$6('font-weight', 'bold'),
    [UNBOLD]: create$6('font-weight', 'normal'),
    [BLUE]: create$6('color', 'blue'),
    [GREEN]: create$6('color', 'green'),
    [GREY]: create$6('color', 'grey'),
    [RED]: create$6('color', 'red'),
    [PURPLE]: create$6('color', 'purple'),
    [ORANGE]: create$6('color', 'orange'), // not well supported in chrome when debugging node with inspector - TODO: deprecate
    [UNCOLOR]: create$6('color', 'black')
  };
  ...
  const addChangedTypeToTransaction = (transaction, type, parentSub) => {
    ...
    if (item === null || (item.id.clock < (transaction.beforeState.get(item.id.client) || 0) && !item.deleted)) {
      setIfUndefined$1(transaction.changed, type, create$4).add(parentSub);
    }
  };
  ...
  const typeListDelete = (transaction, parent, index, length) => {
    ...
    if (length > 0) {
      throw create$3('array length exceeded')
    }
    if (parent._searchMarker) {
   ...
   * @return {boolean}
   */
  const equalFlat$2 = (a, b) => a === b || (length$2(a) === length$2(b) && every$2(a, (val, key) => (val !== undefined || hasProperty$1(b, key)) && b[key] === val));
   ...
   * @return {boolean}
   */
  const equalAttrs = (a, b) => a === b || (typeof a === 'object' && typeof b === 'object' && a && b && equalFlat$2(a, b));
  ...
  class Item extends AbstractStruct {
    ...
    delete (transaction) {
      if (!this.deleted) {
        ...
        setIfUndefined$1(transaction.changed, parent, create$4).add(this.parentSub);
    ...
    if (wsclient.shouldConnect && wsclient.ws === null) {
      ...
      const onclose = error => {
        if (wsclient.ws !== null) {
          ...
          setTimeout(setupWS, min$1(log10(wsclient.unsuccessfulReconnects + 1) * reconnectTimeoutBase, maxReconnectTimeout), wsclient);
        }
        clearTimeout(pingTimeout);
      };
      const sendPing = () => {
   ...
   * @extends Observable<string>
   */
  class WebsocketClient extends Observable$1 {
    /**
     * @param {string} url
     * @param {object} [opts]
   ...
   * @return {Uint8Array}
   */
  const _encodeUtf8Native = str => utf8TextEncoder$1.encode(str);
   ...
   */
  /* istanbul ignore next */
  const encodeUtf8 = utf8TextEncoder$1 ? _encodeUtf8Native : _encodeUtf8Polyfill;
  ...
  const publishSignalingMessage = (conn, room, data) => {
    if (room.key) {
      encryptJson(data, room.key).then(data => {
        conn.send({ type: 'publish', topic: room.name, data: toBase64$1(data) });
      });
    } else {
          ...
          case 'publish': {
              ...
              switch (data.type) {
                case 'announce':
                  if (webrtcConns.size < room.provider.maxConns) {
                    setIfUndefined$1(webrtcConns, data.from, () => new WebrtcConn(this, true, data.from, room));
                    emitPeerChange();
                  }
                  break
                case 'signal':
                  if (data.to === peerId) {
                    setIfUndefined$1(webrtcConns, data.from, () => new WebrtcConn(this, false, data.from, room)).peer.signal(data.signal);
                    emitPeerChange();
                  }
                  break
              }
            ...
            if (room.key) {
              if (typeof m.data === 'string') {
                decryptJson(fromBase64$1(m.data), room.key).then(execMessage);
              }
            } else {
              execMessage(m.data);
            }
  ...
  class WebrtcProvider extends Observable {
    ...
    connect () {
      ...
      this.signalingUrls.forEach(url => {
        const signalingConn = setIfUndefined$1(signalingConns, url, () => new SignalingConn(url));
