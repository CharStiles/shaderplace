...
var promise = require('./promise-a74b7220.cjs');
require('./symbol-c5caa724.cjs');
var logging = require('./logging-dd7751ff.cjs');
var prng = require('./prng-70808343.cjs');
...
const repititionTime = Number(environment.getParam('--repitition-time', '50'));
/* istanbul ignore next */
...
const run = async (moduleName, name, f, i, numberOfTests) => {
  ...
  const times = [];
  const start = isomorphic.performance.now();
  let lastTime = start;
  let err = null;
  isomorphic.performance.mark(`${name}-start`);
  do {
    try {
      const p = f(tc);
    ...
    const currTime = isomorphic.performance.now();
  ...
  isomorphic.performance.mark(`${name}-end`);
  /* istanbul ignore if */
  if (err !== null && err.constructor !== SkipError) {
    logging.printError(err);
  }
  isomorphic.performance.measure(name, `${name}-start`, `${name}-end`);
  logging.groupEnd();
...
const measureTime = (message, f) => {
  ...
  try {
    ...
    duration = isomorphic.performance.now() - start;
...
const runTests = async tests => {
  ...
  const end = isomorphic.performance.now();
  logging.print('');
...
exports.repititionTime = repititionTime;
exports.run = run;
